...
int main(int argc, char **argv) {
	...
	cout << "# ERROR_MODE= " << ERROR_MODE
			<< "//0 for packet error rate ,1 for bit error rate " << endl;
	cout << "# CONTROLE_MODE= " << CONTROLE_MODE
			<< "/ 0 for omission angel, 1 for omission velocity,2 for normal control "
			<< endl;
	cout << "# RMSE_MODE= " << RMSE_MODE
			<< "//0 for difference of the optimal value,1 for difference of the reference\n\n\n"
			<< endl;
...
//ERROR_MODE_0
	...
	double p_bit[] = {			//bit error rate
					...
					0.0001220703125,			//6
					0.000171661376953125, 0.0002460479736328125,
					0.0003681182861328125, 0.0007419586181640625			//7
			};
...
//ERROR_MODE_1
	...
	for (int number_disturbance = 0; number_disturbance < z;
			number_disturbance++) {
		...
		for (int f = 0; f < count_error_index; f++) {
			...
			do {
				...
				do {
					...
					for (i = 0; i < 4; i++)
						w[i] = hrand_gauss(sigma_w[number_disturbance]);
					...
					switch (ERROR_MODE) {
												...
												case 1:
													if (estimate >= 1) {
														noerror_U = (hrand_01()
																>= pbit_per_backward_estimate[f]);
													} else {
														noerror_U = (hrand_01()
																>= pbit_per_backward_not_estimate[f]);
													}
													break;
												}
					...
					if (noerror_X) {
						...
						case 1://omission velocity

							if (estimate >= 1) {
								x_controller_index = k;
								x_controller[k][ARM_ANGLE] = x[k][ARM_ANGLE];
								x_controller[k][ARM_VELOCITY] = (x[k][ARM_ANGLE]-x_controller[k-1][ARM_ANGLE])/Ts;
								x_controller[k][PENDULUM_ANGLE] =x[k][PENDULUM_ANGLE];
								x_controller[k][PENDULUM_VELOCITY] =(x[k][PENDULUM_ANGLE]-x_controller[k-1][PENDULUM_ANGLE])/Ts;
								x_hat[k] = x_controller[k];
								count_reduction_number++;
								estimate++;
							//	cout<<"n"<<endl;
								...
								x_controller_index = k;
								x_controller[k] = x[k];
								cout<<"ARM_ANGLE_x"<<x[k][ARM_ANGLE]<<endl;
								cout<<"PENDULUM_ANGLE_x"<<x[k][PENDULUM_ANGLE]<<endl;
								cout<<"ARM_ANGLE_c"<<x_controller[k][ARM_ANGLE]<<endl;
								cout<<"PENDULUM_ANGLE_c"<<x_controller[k][PENDULUM_ANGLE]<<endl;
								x_hat[k] = x_controller[k];
								estimate++;
								//cout<<"d"<<endl;
							}
...
//cout<<"a"<<endl;
				...
				//	cout<<"b"<<endl;
					falldown = (fabs(x[k + 1][PENDULUM_ANGLE])
							>= Pendulum_Angle_Range);
					if (falldown) {
						break;
					}
			...
			double reductionrate = ((double) count_reduction_number
					* (double) 16)
					/ ((double) loop * (double) Sample_Range * (double) 144);
			count_mse /= ((double) loop - (double) count_falldown)
					* (double) Sample_Range;
			...
			case 0:
				...
				break;
			case 1:
				cout << "#packet error rate(bit error mode)" << "\t"
						<< pbit_per_forward[f] << endl;
				break;
...
//					<< endl;
//			cout << "#falldown rate\t" << count_falldown << "/" << loop << "%"
//					<< endl;
//			cout << "#RMSE\t" << "pendulum_angle\t" << "arm_angel\t"
//					<< "pendulum_velocity\t" << "arm_velocity\t" << "\n"
//					<< endl;
//			cout << "#rad\t" << sqrt(count_mse[PENDULUM_ANGLE]) << "\t"
//					<< sqrt(count_mse[ARM_ANGLE]) << "\t"
//					<< sqrt(count_mse[PENDULUM_VELOCITY]) << "\t"
//					<< sqrt(count_mse[ARM_VELOCITY]) << endl;
//			cout << "#deg\t" << 180 * sqrt(count_mse[PENDULUM_ANGLE]) / M_PI
//					<< "\t" << 180 * sqrt(count_mse[ARM_ANGLE]) / M_PI << "\t"
//					<< sqrt(count_mse[PENDULUM_VELOCITY]) * 180 / M_PI << "\t"
//					<< sqrt(count_mse[ARM_VELOCITY]) * 180 / M_PI << "\n\n\n"
//					<< endl;
//			cout << "#redction rate\t" << setprecision(10)
//					<< count_reduction_number << "\t" << reductionrate
//					<< "%\n\n\n\n" << endl;
